...
import { useDeleteApiByIdMutation } from "@/features/apis/apisApi";
...
const Settings = () => {
  ...
  const navigate = useNavigate();
	...
	const [deleteApiById, { isLoading }] = useDeleteApiByIdMutation();

	const handleDelete = async () => {
		if (projectName === "delete") {
			try {
				const response = await deleteApiById(apiId as string).unwrap();
				console.log("API Project deleted:", response);
	...
	return (
		<div className="space-y-3 px-6 mt-5">
			...
			<p>
				...
				Hub Listing, and destroy your team’s data from Requests, Testing, and
			...
			<AlertDialog>
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Are you sure you want to permanently delete this API Project? This
							action
					...
					<AlertDialogFooter>
						...
						>
							{isLoading ? "Deleting..." : "Delete API Project"}{" "}
							{/* 👈 show loading state */}
						</AlertDialogAction>
					</AlertDialogFooter>
				</AlertDialogContent>
