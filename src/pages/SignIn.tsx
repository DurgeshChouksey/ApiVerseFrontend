  ...
  const [isLoading, setLoading] = useState(false);
    ...
    try {
      setLoading(true);
        ...
        setLoading(false);
				...
				setTimeout(() => navigate("/public"), 1000);
			} else {
			...
			// RTK Query errors are shaped like this:
			// err.data.message OR err.error
			const errorMsg =
				err?.data?.message ||
				err?.error ||
	...
	async function handleGoogleSignin() {
		window.location.href =
			"https://apiverse.durgesh65178.workers.dev/api/v1/auth/google";
	}
						...
						>
							<CardContent className="space-y-6 p-8">
								...
								<motion.div
									...
									whileTap={{ scale: 0.98 }}
								>
									<Button onClick={signinHandler} className="w-full">
										Continue
									</Button>
								</motion.div>
								...
								{successMessage && (
									<p className="text-green-500 underline text-center">
										{successMessage}
									</p>
								)}
                {isLoading && (
								...
								<a
									...
									className="hover:text-primary mb-2 text-center block underline"
								>
									Forgot password?
								</a>
